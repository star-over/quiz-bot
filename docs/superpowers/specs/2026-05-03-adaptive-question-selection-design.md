...
При выходе слот освобождается. Роль для `fillSlot()` выбирается динамически через `chooseRefillRole()`: если все активные (non-consolidated) слоты имеют `known ≥ NEW_KC_KNOWN_THRESHOLD` (0.85), refill становится `"new"`, иначе сохраняется исходная роль слота.
